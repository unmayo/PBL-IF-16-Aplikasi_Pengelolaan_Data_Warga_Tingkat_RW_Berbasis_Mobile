...
import 'package:pendawa/profileCRiwayat.dart';
import 'package:pendawa/settings.dart';
import 'package:pendawa/api.dart';

class MyPengajuanAdminRT extends StatelessWidget {
  const MyPengajuanAdminRT({Key? key});

  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      home: PengajuanAdminRTPage(),
    );
  }
}

class PengajuanAdminRTPage extends StatefulWidget {
  PengajuanAdminRTPage({Key? key});

  @override
  _PengajuanAdminPageState createState() => _PengajuanAdminPageState();
}

class _PengajuanAdminPageState extends State<PengajuanAdminRTPage> {
...
// Inside _RiwayatSuratPageState class
  ...
  Future<void> fetchData() async {
    try {
// Assuming RT().rt is the value you want to send as no_rt
      int noRtValue = RT().rt;

// Use the http.post method with the 'body' parameter
      final response = await http.post(
        Uri.parse(Api.getPengajuanAdminRT),
        body: {
          'no_rt': noRtValue.toString()
                      ...
                      onPressed: () {
                        if (data['status'] != 'Kosong') {
                          ...
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => const ProfilePageCriwayat(),
                            ),
                          );
                        } else {
