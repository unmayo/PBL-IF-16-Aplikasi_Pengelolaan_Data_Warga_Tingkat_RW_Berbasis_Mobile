...
Future<List<Profile>> fetchProfiles() async {
  ...
  final response = await http.post(
    Uri.parse(Api.getKeluargaA),
    body: {'rt': rt.toString()}, // Convert int to String
  );
  ...
  if (response.statusCode == 200) {
    try {
      ...
      List<Profile> profiles = dataList.map((item) {
        return Profile(
          imagePath: item['profile_image'] ?? 'noimage.jpg',
          address: item['alamat_sekarang'] ?? 'no Data',
          name: item['nama_lengkap'] ?? 'no Data',
          gender: item['jenis_kelamin'] ?? 'no Data',
          phoneNumber: item['no_handphone'] ?? 'no Data',
          nik: item['no_ktp'] ?? 'no Data',
        );
      }).toList();
                  ...
                  onPressed: () {
                    if (profile.address != "no Data") {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text('Kosong mas, tidak ada isi :v'),
                        ),
                      );
                    }
                  },
...
class PendataanAdminRTPage extends StatelessWidget {
  const PendataanAdminRTPage({Key? key}) : super(key: key);
